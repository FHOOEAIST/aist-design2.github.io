...
- `_includes/news-card.html` (expects `post`) is shared by the news listing and the home-page teaser. `_includes/project-card.html` (expects `project`, `is_active`) emits a full card for running projects and a compact logo tile for completed ones; both carry `data-category` for the client-side area filter on the projects pages. The completed grid sits in a `.fold` wrapper (`data-fold`, `data-fold-rows`, items in the `data-fold-items` list) that `initFolds()` in `main.js` clips to two rows plus a fading third, with a centred chevron button (`data-fold-toggle`, matched via `aria-controls`) to unfold; the same fold is used per year on the publication/thesis listings. Rows are measured in JS, and the filters re-measure after every change.
